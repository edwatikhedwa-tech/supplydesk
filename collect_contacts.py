...
Примеры:
    ...
    python collect_contacts.py kirpich.ru braer.ru --max-pages 8
    python collect_contacts.py --from-serp results/serp.csv --workers 12 --no-mx
"""
    ...
    if args.web:
        ...
        if not user or not key:
            raise SystemExit(
                ...
                "XMLRIVER_KEY в .env рядом со скриптом."
            )
        else:
            llm = None
            if args.llm:
