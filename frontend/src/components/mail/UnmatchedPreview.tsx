...
export function UnmatchedPreview({ refreshKey, onShowAll, onOpenMessage, onDragStart, onDragEnd }: UnmatchedPreviewProps) {
  ...
  return (
    <section aria-labelledby="unmatched-preview-title" className="border-b border-ink-200 bg-white px-3 py-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2">
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-amber-50 text-amber-700 ring-1 ring-amber-200"><Mail size={14} aria-hidden="true" /></span>
          <div className="min-w-0">
            <h2 id="unmatched-preview-title" className="truncate text-xs font-bold uppercase tracking-wide text-ink-900">Без привязки</h2>
        ...
        <div className="mt-2 space-y-1.5">
          {items.map((item) => (
            <div
              ...
              onDragEnd={onDragEnd}
              className="group flex items-center gap-1 rounded-xl border border-ink-200 bg-white p-1.5 transition-colors hover:border-accent-300 hover:bg-accent-50/60"
