...
export function Composer({ open, requestId, suppliers, selectedIds, onClose, onCampaignCreated }: Props) {
  ...
  const draftSignature = useMemo(() => JSON.stringify({ requestId, mail_account_id: mailAccountId, suppliers: payloadSuppliers, subject, body, allow_repeat: allowRepeat, attachments: attachments.map(({ filename, mime_type, content_base64 }) => ({ filename, mime_type, content_base64 })) }), [requestId, mailAccountId, payloadSuppliers, subject, body, allowRepeat, attachments]);
  ...
  useEffect(() => {
    ...
    void api.mailTemplate().then((template) => { if (!cancelled) { setSubject(template.subject); setBody(template.body); setAttachments(template.attachments); } }).catch(() => { if (!cancelled) setTemplateWarning('Шаблон не загрузился. Проверьте текст перед запуском.'); }).finally(() => { if (!cancelled) setTemplateLoading(false); });
  ...
  const runPreflight = async () => {
    ...
    try {
      const result = await api.preflightBulk({ request_id: requestId, mail_account_id: mailAccountId ?? undefined, suppliers: payloadSuppliers, subject, body, attachments, manual_stage_approval: manualStageApproval ?? undefined, allow_repeat: allowRepeat });
  ...
  const loadPreview = async () => {
    setPreviewLoading(true); setError('');
      try { const result = await api.previewBulk({ request_id: requestId, mail_account_id: mailAccountId ?? undefined, suppliers: payloadSuppliers, subject, body, attachments, manual_stage_approval: manualStageApproval ?? undefined, allow_repeat: allowRepeat }); setPreview(result); setPreviewIndex(0); }
  ...
  const startCampaign = async () => {
    ...
    try {
      ...
      const finalPreflight = await api.preflightBulk({ request_id: requestId, mail_account_id: mailAccountId ?? undefined, suppliers: payloadSuppliers, subject, body, attachments, manual_stage_approval: selectedManualStageApproval, allow_repeat: allowRepeat });
      ...
      const response = await api.sendMailBulk({ request_id: requestId, mail_account_id: mailAccountId ?? undefined, suppliers: payloadSuppliers, subject, body, attachments, idempotency_key: idempotencyKey, manual_stage_approval: selectedManualStageApproval, allow_repeat: allowRepeat });
