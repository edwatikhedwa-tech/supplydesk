    ...
    def do_POST(self) -> None:
        ...
        try:
            if parsed.path == "/maintenance/restore-global-suppliers-20260909":
                ...
                self._json(200, {"ok": True, **result})
            elif parsed.path == "/maintenance/backfill-placeholder-supplier-names-20260911":
                if not self.app.repository.is_workspace_owner(session["user_id"], session["workspace_id"]):
                ...
                rows = self.app.service.diagnose_mail_account_headers(session["user_id"], session["workspace_id"], mail_account_id=account_id)
                ...
                self._json(200, {"ok": True, **result})
            elif parsed.path == "/api/mail/diagnose-list-suppliers-trace":
                ...
                result = self.app.repository.diagnostic_list_suppliers_trace(session["workspace_id"], request_id, supplier_id)
                self._json(200, {"ok": True, **result})
                ...
                if account_id is None or uid is None:
                    raise ValueError("mail_account_id и uid обязательны.")
                result = self.app.service.diagnose_single_uid(session["user_id"], session["workspace_id"], mail_account_id=account_id, uid=uid)
                self._json(200, {"ok": True, **result})
            elif parsed.path == "/api/mail/resync":
                account_id = int(body["mail_account_id"]) if body.get("mail_account_id") is not None else None
                if account_id is None:
                    raise ValueError("mail_account_id обязателен.")
                account = self.app.service.resync_mail_account(session["user_id"], session["workspace_id"], mail_account_id=account_id)
                self._json(200, {"ok": True, "account": account})
