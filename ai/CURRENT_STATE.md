...
и `Отправленных` всех подключённых ящиков, сначала показывает только
метаданные совпадений, а после отдельного подтверждения создаёт один черновик
и переносит полные письма. Поиск не является нечётким: после IMAP-кандидата
SupplyDesk повторно сравнивает всю нормализованную тему; игнорируются только
`Re:`/`Fwd:` и пробелы. Для кириллических тем с валидной `[SD-…]` меткой
добавлен дополнительный ASCII-поиск по метке, поскольку сервер может хранить
Subject в RFC 2047-кодировке. Повторное создание с уже занятой меткой
отклоняется. В live-preview `[SD-1061]проверка запроса` найдено ровно одно
письмо в «Отправленных»; черновик намеренно не создавался. Focused backend/UI
tests, frontend build, HTTP/API smoke и desktop render пройдены. Tablet/mobile
render не проверен: доступный browser bridge не предоставляет смену viewport.
Исправлен дефект multi-recipient Sent: один импортированный исходящий message
теперь создаёт отдельную supplier-связь, thread и waiting-state для каждого
адресата из `To` и `Cc`; focused regression доказывает сценарий с 2 и 60
адресатами. Локальный canonical backend перезапущен, исходящая почта принудительно
выключена launcher-ом.
Повторная живая синхронизация заявки №1063 по её существующей метке добавила
второго адресата из того же исходящего письма: после обновления UI показал
2 компании, 2 контакта, 2 ожидаемых ответа и 2 исходящих сообщения. Это
проверяет backfill уже созданной заявки, а не только новый импорт.
Исправлен переход этой заявки в «Сообщения»: `?request=<id>` переключает
разовый просмотр на «Все» и раскрывает группу заявки, поэтому исходящие со
статусом «Ждём ответа» сразу видны; обычный вход в раздел сохраняет фильтр
«Есть ответ».

`2026-09-13` — локально реализованы `SUP-029A` и ограниченный `SUP-029B`:
у заявки есть неизменяемый workspace-scoped `email_reference` вида `SD-<id>`;
карточка готовит для копирования тему `[SD-<id>] <название>` и сам идентификатор.
Строгий parser принимает только одну корректную метку в квадратных скобках,
а поиск сохраняет границу workspace. Для Яндекс.Почты `Sent` теперь сначала
открывается read-only для preview числа писем с `SD-` в теме; импорт требует
отдельного подтверждения, ограничен 25 письмами и хранит cursor папки и
provider-ID dedupe. Только письмо с корректной меткой текущего workspace может
быть связано с заявкой; сервис не отправляет, не удаляет и не меняет почту.
Владелец может отдельно включить автопоиск в `Sent` для каждого ящика: по
умолчанию он выключен, раз в пять минут ищет только темы с `[SD-` и не читает
обычные личные темы. В live-проверке конкретной `SD-1059` найдено одно письмо;
повторный импорт создал 0 записей, сохранив дедупликацию. Disposable-DB tests,
frontend build и focused source tests прошли. В `EmailRenderer` исправлен
iframe-observer: переписка больше не даёт `MutationObserver` runtime error.
Полная RFC-цепочка, shared mailbox и conflict UI — следующий `SUP-029C`.

Вход снова использует фон `Magic Ring` и три одинаковых круглых почтовых
варианта: Яндекс активен и ведёт на существующий OAuth endpoint, Google и
Mail.ru ясно показаны как disabled «Скоро», потому что их серверной
авторизации в проекте нет. Обычный парольный вход сохранён, а белая карточка
заменена прозрачной композицией на тёмном фоне. Структурные тесты и production
build прошли; новый вариант ожидает live-рендер в анонимной сессии, поскольку
доступная тестовая вкладка уже аутентифицирована и не была принудительно
разлогинена.
