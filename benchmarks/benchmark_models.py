...
Canonical invocation (from the repository root):
    python -m benchmarks.benchmark_models --prepare --from-serp results/serp.csv   # собрать кэш и эталон
    python -m benchmarks.benchmark_models --run                                    # прогнать модели
    python -m benchmarks.benchmark_models --run --models a,b,c                     # свой список

`python benchmark_models.py ...` at the repository root remains a supported
...
def load_truth() -> dict[str, dict]:
    if not TRUTH_PATH.exists():
        raise SystemExit(
            ...
            "Сначала соберите кэш: python -m benchmarks.benchmark_models --prepare --from-serp <csv>"
        )
