...
| REQ-SUPPLIER-001 | `mail/repository.py::upsert_supplier`/`resolve_supplier_for_send` | не написан для этого конкретного случая | Запрос к живой базе данных, 17.09.2026: 28/243 строк | **ПРОВАЛЕНО** — инвариант нарушен, доказательство — прямой подсчёт по базе, а не прошедший тест |
...
| REQ-AI-001 | `chat_service._build_context` + `get_thread_owned` | `tests/test_ai_context_scoping.py` | Юнит-тест проходит | ПРОЙДЕНО |
