...
## Асинхронный анализ и правила порядка (EDW-38)
- Sync только сохраняет и ставит задачи (`mail_analysis_jobs`, миграция 059) в той же транзакции; анализ делает воркер (`run_analysis_jobs`): claim по токену и аренде, retry с паузой, ответ модели сохраняется сразу (`mail_ai_reply_cache`) и воспроизводится после падения — платить второй раз не приходится. Сверка пропущенных задач требует `since` (историю ящика не ставит в очередь).
- Порядок: routing -> request resolution -> extraction. Письмо без цены и без надёжной заявки моделью не анализируется. `acknowledgement` / `pending_quote` — правилами (в собственном тексте нет цифр).
- Факт письма без подтверждённой заявки хранится с `request_id = NULL` (source fact); привязка после ручного решения — `rebind_message_facts`, без модели, аудит в `mail_fact_bindings`.
- Флаг `MAIL_INTELLIGENCE_ON_SYNC` по умолчанию выключен.
