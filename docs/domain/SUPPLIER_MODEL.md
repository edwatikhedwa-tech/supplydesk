...
### 7.3 Приоритет выбора адресата при реальной отправке (2026-09-16)

`mail/repository.py::resolve_supplier_for_send` — единственное место, где
для существующего `suppliers.id` окончательно определяется адрес письма
перед вставкой в `mail_messages` (используется и для новой заявки/кампании,
и для ручного/повторного письма), теперь при наличии `user_id` дополнительно
вызывает `resolve_effective_send_email` и применяет приоритет:
...
fallback-адрес (пункт 3), а сам факт понижения приоритета фиксируется в
`audit_events` (`action='mail.contact_resolution.demoted'`) для
прозрачности, а не применяется молча. Существующая защита от опечатки/
чужого адреса (`stored_email != email → ValueError`) не ослаблена: адрес,
который не совпадает ни с уже сохранённым в `suppliers.email`, ни с
результатом `resolve_effective_send_email`, по-прежнему отклоняется.

Это единственная точка интеграции — `_select_contact_for_request` (выбор
между несколькими уже известными контактами карточки компании, ротация
неиспользованных адресов, проверка «уже ответил»/«уже писали в этой
заявке») и весь preflight/pacing/dedup выше по стеку не изменены и
работают над тем же `item["email"]`, что и раньше; апгрейд происходит
только в момент финального связывания с конкретной `suppliers.id`-строкой.
Доказано `tests/test_contact_resolution_send_path.py` (7 тестов: workspace
preferred побеждает; глобальный preferred используется при отсутствии
override; workspace preferred побеждает глобальный; hard bounce не
выбирается слепо при наличии безопасной альтернативы; при отсутствии
...
той же компании не видит чужой override) плюс полный существующий
...
заполняются никаким пайплайном автоматически. `preflight_bulk` (предпросмотр
кампании до отправки) не показывает апгрейженный адрес — предпросмотр
видит исходный/сохранённый email, а не итоговый; расхождение появляется
только в момент фактической постановки в очередь (`queue_bulk`). См.
