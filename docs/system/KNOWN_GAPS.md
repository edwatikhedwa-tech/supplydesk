...
**Статус (19.09.2026, EDW-13…EDW-17 — Iteration 1 НЕ закрыта):**
- Вторая identity для личного email не создаётся при достаточном evidence (реальный ответ, подтверждение
  пользователя); `rfq_sent` — только связь в заявке. Сходство имени — слабый candidate. Единая модель:
  `docs/domain/SUPPLIER_MODEL.md` §8.
- Готово: единый источник evidence/contact intelligence, состояния candidate/confirmed/rejected/revoked/
  ambiguous и API confirm/reject/revoke, обратимый merge/unmerge с журналом, запрет разных ИНН,
  tenant-тесты, dry-run исторических дублей (на локальной БД: 24 пары, все требуют ручного подтверждения,
  БД не изменена).
- Осталось до закрытия: проверка миграций 053–055 на PostgreSQL; применение merge к историческим дублям
  (только отдельным решением по отчёту); UI review.
- Без evidence отправка на новый личный адрес всё ещё создаёт отдельную карточку.
