...
class MailService:
    ...
    def diagnose_mail_account(self, user_id: int, workspace_id: int, *, mail_account_id: int) -> dict[str, Any]:
        ...
        provider = self._provider_for_account(account, access_token)
        imap_state = provider.diagnostic_inbox_uid_count(account["email"], access_token)
    ...
    def sync_incoming(self, user_id: int, workspace_id: int, *, max_messages: int = 100, mail_account_id: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            self.repository.mark_mail_error(account["id"], "", status="connected")
            # fetch -> persist -> deduplicate -> enqueue happened inside import_incoming_messages; nothing is analysed in the sync path
