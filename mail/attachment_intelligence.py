...
ANALYSIS_VERSION = "attach-extract/v1"
...
def norm(text: str) -> str:
    s = (text or "").lower().replace("гост", "gost").replace(" ", " ")
...
def _fragments_to_line(row: list[dict[str, Any]], gap: float, loc: dict[str, Any]) -> dict[str, Any]:
    ...
    for f in row:
        if cells and f["x0"] - cells[-1]["x1"] < gap:
            ...
            cells.append({"t": f["t"], "x0": f["x0"], "x1": f["x1"], "y0": f["y0"], "y1": f["y1"], "num": None})
    ...
    for pno, page in enumerate(pages, 1):
        ...
        if len(positions) == len(texts):
            for (x, y), (t, size) in zip(positions, texts):
                frags.append({"t": t, "x0": x, "x1": x + len(t) * size * 0.5, "y0": y, "y1": y + size, "yc": -y})
...
_HEADER_CLASSES: list[tuple[str, re.Pattern[str]]] = [
    ...
    ("sku", re.compile(r"артикул|^арт|part\s*no|sku|^код", re.I)),
...
def extract_lines(lines: list[dict[str, Any]], terms: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    tol = 12.0 if pdf_like else 40.0
    price_per = 1
    for ln in lines:
        text = ln["text"]
        h = find_header(ln)
        ...
        if header is not None:
            ...
            qty = qty if qty is not None else parse_number(" ".join(c["t"] for c in qty_cells))
            ...
            total = parse_number(" ".join(c["t"] for c in cols.get("total", []))) if cols.get("total") else None
            cur = terms.get("currency")
            if price is None and _UNPRICED.fullmatch(price_txt.strip() or "-"):
            ...
            if price is None or qty is None:
                ...
                continue
            review = None
            if total is not None and abs(qty * price / price_per - total) > max(0.02 * total, 0.5):
                review = "arithmetic_mismatch"                        # qty x price != stated total: a misread digit or a wrong column
            facts.append(_fact(name, sku, qty, unit, price, cur, ln["loc"], text, price_per, int(deliv) if deliv else None, review))
            continue
        ...
        elif re.search(r"\d[\d\s]*[.,]?\d*\s*(руб|₽|usd|\$)", text, re.I) and not re.search(r"р/с|бик|инн|тел\.", text, re.I):
            unparsed.append({"loc": ln["loc"], "text": text, "header": header_text})
...
AI_SYSTEM = ("Ты читаешь строки коммерческого предложения поставщика, полученные с помощью OCR (возможны ошибки распознавания). Верни ТОЛЬКО JSON по схеме. "
             ...
             "quantity, unit, price (цена за единицу, число). Сумму строки, итог, телефон, ИНН, банковские реквизиты ценой не считай. "
...
def ai_read_rows(models: Models, rows: list[dict[str, Any]], terms: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for it in data.get("items") or []:
        ...
        if not isinstance(i, int) or not 0 <= i < len(rows) or i in seen:
            continue
        row = rows[i]
        nums = {round(n, 2) for n in (parse_number(t) for t in re.findall(r"\d[\d\s]*[.,]?\d*", row["text"])) if n is not None}
        nums |= {round(n, 2) for n in (parse_number(t.replace(" ", "")) for t in re.findall(r"\d[\d\s]*[.,]?\d*", row["text"])) if n is not None}
...
def analyze_attachment(data: bytes, filename: str = "", models: Models | None = None, cache: Any = None, version: str = ANALYSIS_VERSION) -> dict[str, Any]:
...
def match_position(fact: dict[str, Any], positions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    line_txt = f"{fact.get('name') or ''} {fact.get('sku') or ''}"
    win = _windows(line_txt)
    line_sku = _sku_key(fact.get("sku") or "")
    # 1. article evidence: the position's SKU appears as whole tokens of the line
    hits = [p["pid"] for p in positions if p.get("sku") and (_sku_key(p["sku"]) in win and len(_sku_key(p["sku"])) >= 3 or (line_sku and line_sku == _sku_key(p["sku"])))]
    if len(hits) == 1:
    ...
    ls = _stems(line_txt)
    scored = sorted(((len(ls & _stems(f"{p['name']} {p.get('sku') or ''}")) / max(1, len(_stems(f"{p['name']}"))), p["pid"]) for p in positions), reverse=True)
